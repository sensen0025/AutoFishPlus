...
-  **Hypixel 主大厅全自动挂机宏 (`/macro`)**：
  ...
  - **自动返回钓位**：自动前往钓鱼点 (`36, 59, -50`)，就位后自动对准正北方（Yaw = 180.0°, Pitch = 28.0°）抛竿；
...
- 🐉 **Hypixel 主大厅神话鱼智能拉扯系统 (Mythical Fish Combat)**：
  - **自动接管 QTE 状态机**：针对附魔 Mythical Hook 咬钩的神话鱼（Archimedes, Aphrodite, Demeter, Helios, Hades, Nyx, Selene, Zeus 等），自动识别浮漂周围伴随实体与血条状态；
  - **绿色阶段 (REEL)**：以 ~6.6 CPS 稳健快速点按收线，极速削减神话鱼 HP；
  - **红色阶段 (STOP)**：绝对 100% 停手松开右键，彻底杜绝拉力过热导致的**脱钩与爆竿**！
  - **无缝挂起与自愈恢复**：搏斗期间自动挂起常规 AutoFish 声音收抛与 AutoMacro 防挂机位移；神话鱼成功收服后自动延时补抛一竿，平滑恢复挂机！
  - 快捷键 `J`：一键启闭全套自动化（包含普通自动钓鱼 + 神话鱼QTE战斗拉扯 + 原地 Anti-AFK 防挂机微动 + 无浮漂自动补抛）；
  - `/af run` 跑路宏已屏蔽，所有非大地图移动逻辑已全面并入默认 J 键；
  - 快捷键 `'`：呼出图形化配置菜单（支持 ModMenu 集成）；
  - 指令 `/af toggle` / `/af stop`：便捷启闭。
